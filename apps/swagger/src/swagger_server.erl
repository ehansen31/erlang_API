...
-spec child_spec(ID :: any(),
		 #{ip => inet:ip_address(), port => inet:port_number(),
		   ...
		   net_opts => []}) -> supervisor:child_spec().

child_spec(ID,
	   #{ip := IP, port := Port, net_opts := NetOpts} =
	       Params) ->
    AcceptorsPool = (?DEFAULT_ACCEPTORS_POOLSIZE),
    {Transport, TransportOpts} = get_socket_transport(IP,
						      Port, NetOpts),
    LogicHandler = maps:get(logic_handler, Params,
			    ?DEFAULT_LOGIC_HANDLER),
    ExtraOpts = maps:get(cowboy_extra_opts, Params, []),
    CowboyOpts = get_cowboy_config(LogicHandler, ExtraOpts),
    [{env, [{dispatch, Dispatch}]}] = CowboyOpts,
    MapCowboyOpts = #{env => #{dispatch => Dispatch}},
    ranch:child_spec({?MODULE, ID}, AcceptorsPool,
		     Transport, TransportOpts, cowboy_clear, MapCowboyOpts).

get_socket_transport(IP, Port, Options) ->
    Opts = [{ip, IP}, {port, Port}],
    case swagger_utils:get_opt(ssl, Options) of
      SslOpts = [_ | _] ->
	  io:format("using ssl opts"),
	  {ranch_ssl, Opts ++ SslOpts};
      undefined ->
	  io:format("using tcp opts"), {ranch_tcp, Opts}
    end.
...
get_cowboy_config(_LogicHandler, [], Opts) -> Opts;
get_cowboy_config(LogicHandler, [{env, Env} | Rest],
		  Opts) ->
    NewEnv = case proplists:get_value(dispatch, Env) of
	       undefined -> [get_default_dispatch(LogicHandler) | Env];
	       _ -> Env
	     end,
    get_cowboy_config(LogicHandler, Rest,
		      store_key(env, NewEnv, Opts));
get_cowboy_config(LogicHandler, [{Key, Value} | Rest],
		  Opts) ->
    get_cowboy_config(LogicHandler, Rest,
